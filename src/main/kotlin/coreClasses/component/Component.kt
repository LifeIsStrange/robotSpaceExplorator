...
sealed class Component() {
    ...
    open suspend fun sendMessage(networkMissionService: NetworkMissionService, currentStage: String?) {}
...
class Fuel(var quantity: Int = 1000) : Component() {
    override suspend fun sendMessage(
        networkMissionService: NetworkMissionService,
        currentStage: String?
    ) {
        networkMissionService.sendMessage(messageContent = "Component Fuel \"$id\", Stage \"$currentStage\": quantity remaining in the tank is : $quantity", newMessageType = MessageType.Telemetry)
    }
...
class Thruster(
    ...
    override suspend fun sendMessage(
        ...
        networkMissionService.sendMessage(messageContent = "Component Thruster \"$id\", Stage \"$currentStage\": power current level $power, damage level: $damageLevel, heat level: $heatLevel degre celsus", newMessageType = MessageType.Telemetry)
    }
...
class Instrument() : Component() {
    override suspend fun sendMessage(
        ...
        networkMissionService.sendMessage(messageContent = "Component Instrument \"$id\", Stage \"$currentStage\": is ok !", newMessageType = MessageType.Telemetry)
    }
}
...
class ControlSystem() : Component() {
    override suspend fun sendMessage(
        ...
        networkMissionService.sendMessage(messageContent = "Component ControlSystem \"$id\", Stage \"$currentStage\": has no problem to report!", newMessageType = MessageType.Telemetry)
    }
}
...
class PowerPlant(var power: Int = 0, var remainingCapacity: Int = 8000, var damageLevel: Int = 0) : Component() {
    override suspend fun sendMessage(
        ...
        networkMissionService.sendMessage(messageContent = "Component PowerPlant \"$id\", Stage \"$currentStage\": informations: Power : $power, remaining capacity: $remainingCapacity and damageLevel: $damageLevel", newMessageType = MessageType.Data)
    }
